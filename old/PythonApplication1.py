...
try:
    ...
    # client.activate_session(username='admin', password='jim', certificate=None)
    # print("Client Session activated")
    ...
    while True:
        ...
        node_name = "ns=2;s=AW-T4"
        node = client.get_node(node_name)
        print('\nparent node =', node)

        node_name = "ns=2;s=AW-T4.CN1_1_X_GEO"
        node = client.get_node(node_name)
        print('\nnode =', node)
        ...
        node = client.get_node("ns=2;s=AW-T4")
        myNodeChild = node.get_children()
        print("myNodeChild is: ", myNodeChild)
        print("number of child nodes: ", len(myNodeChild))
